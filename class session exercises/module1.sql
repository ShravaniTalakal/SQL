...
CREATE TABLE personal_info (Cust_id int Not Null primary key,
  Name varchar(20) Not Null,
  Country varchar(20) Not Null,
  City varchar(20)); 
...
## Truncate will be used to delete all the rows in table.  
## The structure will not be deleted.
truncate table order_info;

## To change the column name.
alter table order_info change Order_id orderid int;
alter table order_info modify orderid smallint;
describe order_info;
...
alter table personal_info add column gender char(1) after Name;
select * from order_info;
...
alter table order_info_backup add primary key (Order_id);
describe order_info_backup;
commit;

drop table pfbackup;
create table pfbackup like personal_info;
create table pfbackup as select * from personal_info;
select * from pfbackup;
describe personal_info;

select cust_id, name, country from personal_info;

select cust_id, name, city from personal_info
